...
let string_of_structured_constant sc =
  let rec show depth sc =
    if depth > 1 then "..." else
      ...
      | Const_block (tag, struct_consts) -> begin match struct_consts with
        ...
        | [c1; c2] ->
          Printf.sprintf "[%d:%d| %a, %a ]" tag 2 
            ...
            (fun () -> show (depth + 1)) c2
        | c1 :: c2 :: _ ->
          Printf.sprintf "[%d:%d| %a, %a, ... ]" tag (List.length struct_consts)
            (fun () -> show (depth + 1)) c1
            (fun () -> show (depth + 1)) c2
      end   
...
let find globals req slot code_idx =
  match globals with
  | Glob(prims, elems) -> begin match req with
